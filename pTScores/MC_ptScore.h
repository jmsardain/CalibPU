...
// Fri May 22 13:45:07 2026 by ROOT version 6.36.02
// from TTree JetTree/
// found on file: /data/jmsardain/CalibPU/trainings/pu/ckpts/Apr212026/final.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Double_t        r_e_calculated[51];   //[nr_e_calculated]
   Int_t           nzL;
   Double_t        zL[59];   //[nzL]
   Int_t           nzRel;
   ...
   Int_t           npredicted_response;
   Double_t        predicted_response[51];   //[npredicted_response]
   Int_t           nerror_on_response;
   Double_t        error_on_response[51];   //[nerror_on_response]
   ...
   TBranch        *b_nodes_out;   //!
   TBranch        *b_npredicted_response;   //!
   TBranch        *b_predicted_response;   //!
   TBranch        *b_nerror_on_response;   //!
   TBranch        *b_error_on_response;   //!

   MC_ptScore(TTree *tree=0);
   virtual ~MC_ptScore();
...
#ifdef MC_ptScore_cxx
MC_ptScore::MC_ptScore(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/data/jmsardain/CalibPU/trainings/pu/ckpts/Apr212026/final.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/data/jmsardain/CalibPU/trainings/pu/ckpts/Apr212026/final.root");
      }
...
MC_ptScore::~MC_ptScore()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t MC_ptScore::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t MC_ptScore::LoadTree(Long64_t entry)
{
...
void MC_ptScore::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("npredicted_response", &npredicted_response, &b_npredicted_response);
   fChain->SetBranchAddress("predicted_response", predicted_response, &b_predicted_response);
   fChain->SetBranchAddress("nerror_on_response", &nerror_on_response, &b_nerror_on_response);
   fChain->SetBranchAddress("error_on_response", error_on_response, &b_error_on_response);
   Notify();
}

bool MC_ptScore::Notify()
{
...
void MC_ptScore::Show(Long64_t entry)
{
...
Int_t MC_ptScore::Cut(Long64_t entry)
{
